...
describe('child touch-target product invariant', () => {
  ...
  it('keeps compact navigation and representative child interaction families at the 44px floor', () => {
    ...
    expect(policy).toMatch(/#app :is\(\.home-button,\.panel-back\) \{[\s\S]*?width: var\(--child-touch-target-min\);[\s\S]*?height: var\(--child-touch-target-min\);[\s\S]*?\}/);
    ...
    expect(policy).toMatch(/#app :is\([^)]+\.sequence-order__move\) \{[\s\S]*?min-width: var\(--child-touch-target-min\);[\s\S]*?min-height: var\(--child-touch-target-min\);[\s\S]*?\}/);
  });
