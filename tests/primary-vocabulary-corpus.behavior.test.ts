...
describe('grade-aware primary vocabulary corpus', () => {
  ...
  it('builds profile review slices without copying child or source definitions into the bridge', () => {
    const syntheticCorpus = {
      ...
      entries: [
        {
          id: 'source.alpha', word: 'alpha', lemma: 'alpha', partOfSpeech: 'noun', grade: 2,
          frequency: { zipf: 5, perMillion: 10, band: 1 },
          gradeEvidence: { reason: null, cefrLevel: 'A1', yleLevel: null, tags: ['source:test'] },
          reviewStatus: 'candidate'
        },
        {
          id: 'source.beta', word: 'beta', lemma: 'beta', partOfSpeech: 'adjective', grade: 2,
    ...
    const knowledge = [{
      ...
      entries: [{
        rowId: 'kr.vocab.meaning.alpha.safe',
        relation: 'means',
        subject: { id: 'alpha', label: 'alpha' },
        object: { id: 'alpha-object', label: 'this prose must never enter the profile bridge' }
      }]
    }, {
      authoring: { status: 'draft' },
      entries: [{
        rowId: 'kr.vocab.meaning.beta.draft',
        relation: 'means',
        subject: { id: 'beta', label: 'beta' },
        object: { id: 'beta-object', label: 'draft prose must stay out too' }
      }]
    }];
    const membership = {
      profileRef: 'TEST_PRIMARY_PROFILE',
      members: [{ rowId: 'kr.vocab.meaning.alpha.safe', fit: 'review' }]
    };
    ...
    expect(slice.summary).toEqual({
      ...
      reviewedCandidateWords: 1,
      alreadyInProfileWords: 1,
      alreadyInProfileRows: 1,
      pendingEditorialWords: 1
    });
    expect(slice.readyForProfileReview).toEqual([expect.objectContaining({
      lemma: 'alpha',
      sourceCorpusId: 'source.alpha',
      kidsplayRowIds: ['kr.vocab.meaning.alpha.safe'],
  ...
  it('connects a real grade/profile selection to existing reviewed vocabulary rows without changing membership semantics', () => {
    ...
    expect(slice.sourceGradeCorpus).toMatchObject({
      sourceId: 'grundwortschatz-voc-en',
      license: 'CC-BY-SA-4.0'
    });
    expect(slice.readyForProfileReview.every((item: any) =>
      item.kidsplayRowIds.every((rowId: string) => rowId.startsWith('kr.vocab.meaning.'))
    )).toBe(true);
