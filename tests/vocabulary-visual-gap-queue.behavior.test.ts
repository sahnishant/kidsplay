...
describe('#88 priority vocabulary visual gap queue', () => {
  it('contains only unaudited priority-corpus candidates and never creates approved senses', () => {
    ...
    expect(queue.items.every((item: { policy: Record<string, boolean> }) =>
      item.policy.senseApproved === false &&
      item.policy.visualStrategyApproved === false &&
      item.policy.profilePlacementInferred === false &&
      item.policy.childDefinitionImported === false
    )).toBe(true);
  });
  ...
  it('preserves review traceability, ambiguity and reusable-template hints without source prose', () => {
    ...
    expect(queue.items.every((item: {
      ...
      candidateIds?: string[];
      polysemyRisk?: string;
      existingStrategyCandidates?: string[];
      motionPotential?: string;
      visualReviewScore?: number;
    }) =>
      Boolean(item.sourceCorpusId) &&
      Number.isInteger(item.grade) &&
      Boolean(item.partOfSpeech) &&
      Boolean(item.corpusProvenance?.sourceId) &&
      Boolean(item.corpusProvenance?.sourceRevision) &&
      Boolean(item.corpusProvenance?.license) &&
      Number.isInteger(item.candidateSenseCount) &&
      Array.isArray(item.candidateIds) &&
      item.candidateIds.length === item.candidateSenseCount &&
      allowedRisks.has(String(item.polysemyRisk)) &&
      Array.isArray(item.existingStrategyCandidates) &&
      item.existingStrategyCandidates.length > 0 &&
      allowedMotion.has(String(item.motionPotential)) &&
      Number.isFinite(item.visualReviewScore)
    )).toBe(true);

    expect(JSON.stringify(queue)).not.toContain('definition');
    expect(JSON.stringify(queue)).not.toContain('examples');
    expect(JSON.stringify(queue)).not.toContain('childDefinition');
  ...
  it('is sorted deterministically by review score, grade and lemma', () => {
    ...
    const keys = queue.items.map((item: { visualReviewScore: number; grade: number; lemma: string }) => ({
      score: item.visualReviewScore,
      grade: item.grade,
      lemma: item.lemma
    }));
    const sorted = [...keys].sort((left, right) =>
      right.score - left.score || left.grade - right.grade || left.lemma.localeCompare(right.lemma)
    );
