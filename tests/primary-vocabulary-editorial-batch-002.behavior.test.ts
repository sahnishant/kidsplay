...
describe('primary vocabulary editorial batch 002 preparation', () => {
  it('deterministically selects the next 16 Grade 1/2 words after terminal batch-001 human review', () => {
    for (const grade of [1, 2]) {
      ...
      const productionReviewPath = `content/lexicon/reviews/grade-${grade}-batch-002.json`;
      const slice = readJson(slicePath);
      const review = readJson(reviewPath);
      const productionReview = readJson(productionReviewPath);
      ...
      expect(packet.summary).toMatchObject({ words: 16, excludedPriorReview: 8, reviewed: 0, accepted: 0, rejected: 0 });
      ...
      const terminalOrHeld = [
        ...productionReview.decisions.map((decision: any) => decision.lemma),
        ...productionReview.unresolvedItems.map((item: any) => item.lemma)
      ].sort();
      expect(terminalOrHeld).toEqual([...expected].sort());
    }
  });
