<script lang="ts">
  ...
  const startEntry = $derived(workshop ?? topic ?? visible[0]);
...
<main class="prototype-home" data-nav100-prototype="true">
  ...
  {:else}
    {#if startEntry}
      <section class="continue-card" aria-label="Start here">
        <div>
          <span class="eyebrow">START HERE</span>
          <h2>{startEntry.childTitle}</h2>
          <p>Open a full activity, or choose another kind below.</p>
        </div>
        <button data-canonical-id={startEntry.canonicalId} type="button" onclick={() => onSelect(startEntry)}>Open</button>
      </section>
    {/if}
...
<style>
  .prototype-home{width:min(940px,100%);min-height:calc(100dvh - 42px);margin:auto;padding:clamp(14px,3vw,28px);display:flex;flex-direction:column;gap:16px;box-sizing:border-box;background:linear-gradient(180deg,#fff8ea 0%,#f5fbff 100%);color:#17324d}.prototype-home__topbar{display:flex;justify-content:space-between;align-items:flex-start}.prototype-tag,.eyebrow{font-size:.72rem;font-weight:800;letter-spacing:.08em}.prototype-tag{display:inline-block;padding:5px 8px;border-radius:999px;background:#fff;border:2px solid #17324d}.prototype-home h1{font-size:clamp(1.55rem,5vw,2.4rem);line-height:1.05;margin:.5rem 0 0;max-width:16ch}.continue-card{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;padding:18px;border-radius:22px;background:#17324d;color:white;box-shadow:0 8px 0 rgba(23,50,77,.14)}.continue-card h2{margin:.25rem 0 .1rem;font-size:1.35rem}.continue-card p{margin:0;opacity:.9}.continue-card button,.prototype-card button{min-height:48px;padding:0 18px;border:0;border-radius:16px;font:inherit;font-weight:800;background:white;color:#17324d;cursor:pointer}.lane-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.lane-card{min-height:118px;padding:16px;border:2px solid rgba(23,50,77,.16);border-radius:22px;background:white;color:inherit;text-align:left;display:grid;grid-template-columns:auto 1fr auto;gap:12px;align-items:center;cursor:pointer;box-shadow:0 5px 0 rgba(23,50,77,.08)}.lane-card:disabled{opacity:.45;cursor:not-allowed}.lane-card__emoji{font-size:2rem}.lane-card__copy{display:flex;flex-direction:column;gap:5px}.lane-card__copy strong{font-size:1.05rem}.lane-card__copy small{font-size:.86rem;line-height:1.25;color:#46627d}.lane-card__arrow{font-size:2rem}.browse-all{width:100%;min-height:64px;padding:12px 18px;border:2px dashed #5b7690;border-radius:18px;background:rgba(255,255,255,.72);color:#17324d;display:flex;justify-content:space-between;align-items:center;font:inherit;cursor:pointer}.prototype-card--status{padding:22px;border-radius:20px;background:white;border:2px solid rgba(23,50,77,.14);display:flex;flex-direction:column;gap:10px}.lane-card:focus-visible,.browse-all:focus-visible,.continue-card button:focus-visible,.prototype-card button:focus-visible{outline:4px solid #ffb02e;outline-offset:3px}@media(min-width:561px){.lane-card:last-child{grid-column:1/-1}}@media(max-width:560px){.prototype-home{gap:12px;padding:12px}.lane-grid{grid-template-columns:1fr}.lane-card{min-height:82px}.continue-card{grid-template-columns:1fr}.continue-card button{width:100%}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important}}
</style>
