<script lang="ts">
  ...
  function challengeLabel(mission: StoryMission): string {
    if (mission.worldActionRef) return `${worldDepthLabel(mission)} Level ${mission.worldDepthLevel ?? 1}`;
  ...
  function missionStartLabel(mission: StoryMission): string {
    return mission.worldActionRef
      ? `Start ${worldDepthLabel(mission)} Level ${mission.worldDepthLevel ?? 1}`
  ...
  function locationAriaLabel(location: StoryLocation, state: 'complete' | 'current' | 'available' | 'locked', mission?: StoryMission): string {
    ...
    const depth = mission?.worldActionRef ? `, ${worldDepthLabel(mission)} depth ${mission.worldDepthLevel ?? 1}` : '';
