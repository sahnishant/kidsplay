<script lang="ts">
  ...
  function pickRain(): void {
    if (currentStep?.id !== 'town.l2.step.clear-rain-channel' || stepComplete) return;
    rainPicked = true;
    feedback = 'Leaves picked up. Put them on the glowing dry bank.';
  }

  function clearRain(): void {
    if (currentStep?.id !== 'town.l2.step.clear-rain-channel' || stepComplete || !rainPicked) return;
    finish(currentStep);
  }
    ...
    <main class="body">
          ...
          <div class="scene-node rain-node" class:active={stepIndex === 4} class:resolved={completedCount > 4}>
            ...
            {#if completedCount <= 4}
              <button
                ...
                onpointerdown={(event) => { if (!stepComplete) { rainPicked = true; rainDrag = beginSimpleDrag(event); } }}
                onpointermove={(event) => { rainDrag = moveSimpleDrag(event, rainDrag); }}
                onpointerup={endRainDrag}
                onpointercancel={() => { rainDrag = null; }}
                onclick={pickRain}
              >❧❧</button>
              <button type="button" class="rain-bank-target" data-rain-target="clear-bank" aria-label="Dry bank. Put the leaves here." aria-disabled={!rainPicked} onclick={clearRain}>↙</button>
            {:else}
      ...
      <section class="action" class:complete={stepComplete} aria-labelledby="town-action-heading">
        ...
        {:else if currentStep.id === 'town.l2.step.clear-rain-channel' && !stepComplete}
          <div class="practical-guide rain-guide"><strong>Clear the actual drain.</strong><p>Drag the leaves onto the dry bank, or tap the leaves and then the bank. Water starts moving only when the channel is open.</p></div>
        {/if}
        ...
        {#if stepComplete}<button type="button" class="primary next-job" onclick={nextStep}>{completedCount === adventure.steps.length ? 'See changed town' : 'Next town job'} <span aria-hidden="true">→</span></button>{/if}
      </section>
    </main>
  {/if}
</section>
...
<style>
  ...
  @media(max-width:650px){.town-depth{height:auto;min-height:calc(100dvh - 42px);overflow:visible}.mission-bar{position:sticky;top:0;z-index:30;padding:6px 8px;background:#fff7dbed;backdrop-filter:blur(8px)}.job-rail{position:sticky;top:58px;z-index:29;padding:3px 8px 6px;background:#fff7dbed;backdrop-filter:blur(8px)}.body{grid-template-columns:1fr;gap:8px;padding:0 8px 10px;overflow:visible}.world,.action{overflow:visible;border-radius:16px}.scene{height:270px;min-height:270px}.building b{display:none}.crossing-node{left:29%;bottom:2%;transform:scale(.75);transform-origin:left bottom}.sign-node{left:15%;top:38%;transform:scale(.82)}.recycle-node{right:4%;top:45%;transform:scale(.8)}.parcel-node{left:4%;bottom:0;transform:scale(.8);transform-origin:left bottom}.parcel-node .help-table{height:55px}.parcel-node .parcel{width:55px;height:55px}.rain-node{right:6%;bottom:0;transform:scale(.9);transform-origin:right bottom}.rain-node .leaf-blockage{min-width:50px;min-height:50px}.rain-node .rain-bank-target{width:50px;height:50px}.scene-node.active{animation:none;filter:drop-shadow(0 0 13px #ffd75c)}.town-slot--sign-top{left:14%;top:36%}.town-slot--sign-base{left:15%;top:46%}.town-slot--paper-bin{right:16%;top:47%}.town-slot--recycling-bin{right:10%;top:47%}.town-slot--compost-bin{right:4%;top:47%}.action{padding:10px}.piece-card{min-height:62px}.completion{margin:0 8px 10px}}
