...
<main class="workshop" data-workshop-section={section.id} data-workshop-idea={beat.id}>
  <header class="workshop__topbar">
    ...
    <span class="workshop__count" aria-label={`Part ${activeIndex + 1} of ${guide.sections.length}, idea ${activeBeatIndex + 1} of ${section.beats.length}`}>
  ...
  <section class="workshop__body" aria-labelledby="workshop-section-title">
    <article class="workshop__lesson">
      ...
      <div class="workshop__look">
        ...
        <div><strong>LOOK</strong><p>{section.lookPrompt}</p></div>
      </div>
      ...
      <div class="workshop__learn">
        ...
        <section class="workshop__beat" data-claim-count={beat.claimRefs?.length ?? 0} data-capability-count={beat.capabilityRefs?.length ?? 0} aria-live="polite">
          <header class="workshop__beat-heading">
            ...
            <div><small>LEARN · IDEA {activeBeatIndex + 1}</small><strong>{beat.label}</strong></div>
          </header>
          <p>{beat.text}</p>
          ...
          {#if beat.examples?.length}
            <div class="workshop__examples" aria-label={`${beat.label} examples`}>
              {#each beat.examples as example}<span>{example}</span>{/each}
            </div>
          {/if}
        </section>
      ...
      {#if isLastBeat}
        <aside class="workshop__try" aria-label="Your turn without scoring">
          ...
          <div><strong>YOUR TURN</strong><p>{section.childPrompt}</p><small>No score here — just explore.</small></div>
        </aside>
      {/if}
    </article>
  </section>
