...
<main class="workshop" data-workshop-section={section.id} data-workshop-idea={beat.id}>
  ...
  <section class="workshop__body" aria-labelledby="workshop-section-title">
    <article class="workshop__lesson" class:workshop__lesson--movement={section.id === 'movement'}>
      ...
      <div class="workshop__learn">
        ...
        <section class="workshop__beat" data-claim-count={beat.claimRefs?.length ?? 0} data-capability-count={beat.capabilityRefs?.length ?? 0} aria-live="polite">
          ...
          <p>{beat.text}</p>
          {#if beat.sequence?.length && section.id !== 'movement'}
...
<style>
  ...
  @media(max-width:650px){.workshop__steps{overflow-x:auto;grid-template-columns:repeat(7,minmax(52px,1fr))}.workshop__step{grid-template-columns:1fr;gap:1px}.workshop__lesson,.workshop__lesson--movement{grid-template-columns:1fr;grid-template-rows:auto auto auto auto auto;padding:8px}.workshop__lesson-heading,.workshop__look,.workshop__try{grid-column:1}.workshop__visual{min-height:155px;max-height:27dvh}.workshop__visual--mechanism{min-height:330px;max-height:none}}
</style>
