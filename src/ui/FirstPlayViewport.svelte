...
>
  ...
  <section style="min-height:0;display:grid;grid-template-rows:auto auto minmax(0,1fr) auto;gap:6px;overflow:hidden">
    <div style="display:flex;align-items:center;justify-content:center;gap:8px;min-height:58px">
      ...
      {:else}
        <p style="margin:0;max-width:28ch;text-align:center;font-weight:900">{currentPrompt}</p>
      {/if}
    </div>
    ...
    <div style="min-height:0;display:grid;align-items:center;overflow:auto">
      {#if current && mode === 'first_play'}
        ...
        {:else if activity.kind === 'cause_effect'}
          <button class="choice-button cause-effect-target" style="width:100%;min-height:300px;padding:8px" type="button" data-first-play-primary="true" aria-label={causeEffectFilled ? 'Full bucket' : 'Empty bucket'} disabled={complete} onclick={() => { if (!complete) { causeEffectFilled = true; finish('change'); } }}><ContainerStateVisual state={causeEffectFilled ? activity.afterState : activity.beforeState} /></button>
        {/if}
