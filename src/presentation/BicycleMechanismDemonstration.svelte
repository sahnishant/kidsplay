<script lang="ts">
  import bikeUrl from '../ui/bicycleStoryBike.svg?url';
  ...
  function advance(): void {
    stepIndex = isLast ? 0 : stepIndex + 1;
  }
</script>
  ...
  <div class="mechanism__diagram" role="img" aria-label={mode === 'drive' ? 'The same colorful bicycle showing pedal, crank, chain and back wheel movement' : 'The same colorful bicycle showing the brake lever, brake and slowing wheel'}>
    <div class="mechanism__art" class:rolling={mode === 'drive' && current.id === 'roll'}>
      ...
      <svg class="mechanism__overlay" viewBox="0 0 760 420" aria-hidden="true">
        <defs>
          <marker id="mechanism-arrow" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto">
        ...
        {#if mode === 'drive'}
          <g class="mechanism-crank" class:active={current.id === 'crank' || current.id === 'pedal'} class:turning={stepIndex >= 1}>
            <circle cx="355" cy="280" r="39" />
            <line x1="355" y1="280" x2="398" y2="252" />
            <line x1="355" y1="280" x2="315" y2="307" />
          </g>

          <g class="mechanism-pedal" class:active={current.id === 'pedal'}>
            <rect x="389" y="243" width="44" height="20" rx="9" />
            <circle class="focus-halo" cx="410" cy="253" r="27" />
          </g>

          <g class="mechanism-chain" class:active={current.id === 'chain'} class:moving={stepIndex >= 2}>
            <path d="M218 260 L350 253" />
            <path d="M218 299 L350 306" />
          </g>

          <g class="mechanism-wheel" class:active={current.id === 'rear-wheel'} class:spinning={stepIndex >= 3}>
            <circle cx="205" cy="280" r="105" />
            <line x1="205" y1="171" x2="205" y2="190" />
          </g>

          <g class="callout pedal-callout" class:active={current.id === 'pedal'}>
            <path d="M412 261 L535 326" />
            <rect x="528" y="306" width="205" height="48" rx="13" />
            <text x="546" y="336">PEDAL · foot pushes</text>
          </g>
          <g class="callout crank-callout" class:active={current.id === 'crank'}>
            <path d="M362 250 L480 186" />
            <rect x="450" y="145" width="218" height="48" rx="13" />
            <text x="468" y="175">CRANK · turns around</text>
          </g>
          <g class="callout chain-callout" class:active={current.id === 'chain'}>
            <path d="M280 294 L244 335" />
            <rect x="112" y="328" width="190" height="48" rx="13" />
            <text x="130" y="358">CHAIN · moves</text>
          </g>
          <g class="callout wheel-callout" class:active={current.id === 'rear-wheel'}>
            <path d="M151 198 L103 136" />
            <rect x="24" y="88" width="205" height="48" rx="13" />
            <text x="42" y="118">BACK WHEEL · turns</text>
          </g>
          ...
          {/if}
        {:else}
          <g class="mechanism-lever" class:active={current.id === 'lever'}>
            <path d="M526 104 Q548 112 552 126" />
            <circle class="focus-halo" cx="535" cy="110" r="28" />
          </g>
          <g class="mechanism-brake" class:active={current.id === 'brake'}>
            <path d="M526 120 C535 145 535 178 520 210" />
            <circle class="focus-halo" cx="520" cy="210" r="28" />
          </g>
          <g class="mechanism-wheel" class:active={current.id === 'slow'} class:slowing={current.id === 'slow'}>
            <circle cx="545" cy="280" r="105" />
            <line x1="545" y1="171" x2="545" y2="190" />
          </g>

          <g class="callout lever-callout" class:active={current.id === 'lever'}>
            <path d="M535 111 L648 147" />
            <rect x="570" y="145" width="168" height="48" rx="13" />
            <text x="588" y="175">LEVER · squeeze</text>
          </g>
          <g class="callout brake-callout" class:active={current.id === 'brake'}>
            <path d="M518 209 L625 244" />
            <rect x="555" y="241" width="183" height="48" rx="13" />
            <text x="573" y="271">BRAKE · slows wheel</text>
          </g>
