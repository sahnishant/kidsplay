...
const gap = readJson('content/vocabulary-visuals/__generated-priority-gap.json');
if (gap?.schemaVersion !== 1 || gap?.issueRef !== 88 || gap?.status !== 'generated_review_queue_only') {
  throw new Error('Batch 002 requires the deterministic #88 priority gap queue');
}
...
const addGap = (lemma, strategy, sceneTemplate = null, parameters = null, motionPolicy = 'none', answerSafety = 'post_answer_only') => {
  ...
  if (!candidate) throw new Error(`${lemma}: reviewed batch catalog entry is absent from the #88 priority gap`);
  if (candidate.polysemyRisk !== 'low' || candidate.candidateSenseCount !== 1 || candidate.candidateIds?.length !== 1) {
    throw new Error(`${lemma}: automatic batch-002 admission requires exactly one pinned candidate identifier`);
  }
...
const addHumanReviewed = (lemma, expectedSenseKey, strategy, sceneTemplate, parameters, motionPolicy = 'none', answerSafety = 'post_answer_only') => {
  ...
  if (!entry || curation?.status !== 'reviewed' || curation?.candidateId !== expectedSenseKey || curation?.sourceGlossCopied !== false) {
    throw new Error(`${lemma}: bounded runtime candidate must resolve to the exact human-reviewed #51 sense without copied source gloss`);
  }
...
const PLACE_LEMMAS = [
  'district', 'hotel', 'beach', 'valley', 'museum', 'restaurant', 'kitchen', 'airport', 'factory', 'campus',
  'highway', 'bedroom', 'cave', 'classroom', 'zoo', 'cafe', 'pond', 'hometown', 'backyard', 'nursery',
  'corridor', 'isle', 'gateway', 'porch', 'cemetery'
];
for (const lemma of PLACE_LEMMAS) addGap(lemma, 'place_scene', 'place', { placeKind: lemma });

const ROLE_LEMMAS = [
  'artist', 'mayor', 'singer', 'employee', 'actress', 'tourist', 'employer', 'creator', 'poet', 'scientist',
...
// Their visual strategies may be reviewed here without importing source glosses or inferring profile placement.
addHumanReviewed('ask', 'ask#v#2', 'action_scene', 'actor-action', { action: 'ask-question' }, 'optional_meaningful');
addHumanReviewed('find', 'find#v#3', 'action_scene', 'actor-action', { action: 'search-and-find' }, 'optional_meaningful');
addHumanReviewed('floor', 'floor#n#1', 'part_whole', 'part-focus', { wholeRef: 'room', partRef: 'floor' });
addHumanReviewed('guide', 'guide#n#2', 'person_role', 'person-role', { role: 'guide' }, 'optional_meaningful');
addHumanReviewed('environment', 'environment#n#2', 'place_scene', 'place', { placeKind: 'living-surroundings' });
addHumanReviewed('minute', 'minute#n#1', 'sequence_scene', 'sequence', { relation: 'sixty-seconds' });
addHumanReviewed('notice', 'notice#n#1', 'diagrammatic', 'simple-diagram', { diagramKind: 'information-notice' });
...
if (sceneGrammarItems / items.length < 0.75) throw new Error('Batch 002 must reuse existing scene grammar for at least 75% of reviewed senses');
...
const output = {
  ...
  reviewBasis: {
    ...
    rule: 'Single-candidate queue rows are admitted only with their exact pinned candidate identifier. Seven otherwise-polysemous rows use exact #51 human-reviewed candidate ids. No source gloss is copied.'
  },
  policy: {
...
console.log(`Built #88 reviewed batch 002: ${items.length} exact sense strategy item(s); scene-grammar reuse ${sceneGrammarItems}/${items.length}; textual-only ${textualOnlyItems}; human-reviewed exact senses ${humanReviewedItems}.`);
