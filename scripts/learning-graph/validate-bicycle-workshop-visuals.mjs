...
export function validateBicycleWorkshopVisuals() {
  ...
  const allVisuals = readdirSync(resolve(ROOT, 'content/visuals')).filter((name) => name.endsWith('.json')).sort().flatMap((name) => read(`content/visuals/${name}`));
  const allAnimations = readdirSync(resolve(ROOT, 'content/animations')).filter((name) => name.endsWith('.json')).sort().flatMap((name) => read(`content/animations/${name}`));
  ...
  for (const question of readQuestions()) for (const item of presentableItems(question)) for (const visualRef of item.visualRefs ?? []) {
  ...
  for (const required of ['entity.transport.bicycle','entity.bicycle-part.pedal','entity.bicycle-part.brake','entity.bicycle-part.handlebar','entity.safety.helmet','animation.variant.bicycle-workshop-neutral']) invariant(allVisualIds.has(required), `Missing canonical visual ${required}`);

  const customRendererUsed = allVisuals.some((visual) => visual.renderer === 'bicycle-workshop-icon');
  invariant(customRendererUsed === false, 'Bicycle Workshop must not add a parallel content renderer');
  invariant(viewport.includes('BicycleStoryStage') && viewport.includes('data-visual-ref'), 'Bicycle Workshop does not project its canonical visual authority through the interactive story stage');
  invariant(stage.includes('bicycleStoryBike.svg?url'), 'Bicycle Workshop story stage is missing its original vector asset');
  invariant(bikeAsset.includes('<svg') && bikeAsset.includes('circle') && bikeAsset.includes('path'), 'Bicycle Workshop original vector asset is malformed or empty');
  invariant(!viewport.includes("from '../presentation/VisualEntity.svelte'") && !viewport.includes("from '../presentation/SemanticAnimation.svelte'"), 'Bicycle Workshop bypasses the approved projection boundary');
  ...
  const registeredWorkshopVisualCount = allVisuals.filter((visual) => visual.id.startsWith('entity.bicycle-part.') || visual.id.startsWith('entity.bicycle-step.') || visual.id.startsWith('animation.variant.bicycle-workshop.')).length;
  return {
