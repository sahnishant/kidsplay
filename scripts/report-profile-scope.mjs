...
const resolver = createMembershipResolver(collections);
...
const families = (target.families ?? []).map((family) => {
  ...
  const groups = (family.requiredGroups ?? []).map((group) => {
    ...
    const inheritedRows = [...effectiveMemberRows]
    ...
    return {
      ...
      directRows,
      includedRows: inheritedRows,
      directReuseCandidates
    };
  });
  const directCurrentRows = [...new Set(groups.flatMap((group) => group.directRows))].sort();
  const inheritedCurrentRows = [...new Set(groups.flatMap((group) => group.includedRows))].sort();
  ...
  const previousClassRows = [...effectiveMemberRows]
    ...
    .sort();
  const inheritedPreviousClassRows = previousClassRows.filter((rowId) =>
    effectiveMembersByRow.get(rowId)?.origin === 'inherited'
  );
  ...
  return {
    ...
    directCurrentRows,
    includedCurrentRows: inheritedCurrentRows,
    previousClassRows,
    includedPreviousClassRows: inheritedPreviousClassRows,
...
const sections = [...new Set(families.map((family) => family.section))].map((section) => {
  ...
  return {
    ...
    missingCurrentGroups: sectionFamilies.flatMap((family) => family.missingGroups.map((group) => `${family.id}:${group.id}`)),
...
console.log(`Inherited profiles: ${summary.includedProfileRefs.join(', ') || 'none'}`);
console.log(`Provenance: ${target.provenance?.status ?? 'unknown'} — ${target.provenance?.placementBasis ?? 'unspecified'}`);
if (target.level1Mix) {
  console.log(`Level 1 scope mix: current class ${target.level1Mix.currentClassPercent}% / previous class ${target.level1Mix.previousClassPercent}%${target.level1Mix.achieversCurrentClassOnly ? '; Achievers current-class only' : ''}`);
}
console.log('A compound family is complete only when every required subgroup has a direct Class 3/shared canonical placement. Inherited previous-class rows are reported separately.');
console.log('');
for (const section of sections) {
  ...
  for (const family of families.filter((item) => item.section === section.section)) {
    ...
    console.log(`- ${family.label}: ${family.currentClassRepresented ? 'represented' : `GAP: ${missing}`}; direct=${family.directCurrentRows.length}; inherited-matching=${family.includedCurrentRows.length}`);
  }
  console.log('');
}
console.log('## Missing required groups and canonical reuse candidates');
if (!summary.missingCurrentGroups.length) {
  ...
  for (const group of summary.missingCurrentGroups) {
    ...
    for (const rowId of group.includedRows.slice(0, 8)) console.log(`  - inherited row eligible for explicit direct reuse: ${rowId}`);
